...
class Drone(ap.Agent):
    def setup(self):
        ...
        self.yolo_model = YOLO('yolov8n.pt')  # Load pre-trained YOLO model
        self.detected_objects = []

    def detect_objects(self, image=None):
        """
        Detect objects using YOLO model with custom robber detection
        """
        if image is None:
            # Create a black image representing the drone's view
            image = np.zeros((416, 416, 3), dtype=np.uint8)
            # Optionally add some simulated objects
        ...
        for result in results:
            ...
            for box in boxes:
                ...
                # Only store objects with confidence > 0.5
                # Customize to detect robbers specifically
                if conf > 0.5 and cls.lower() == 'robber':
        ...
        if self.detected_objects:
            print(f"Drone {self.id} detected: {self.detected_objects}")
        else:
            print(f"Drone {self.id} detected no objects.")

        return self.detected_objects

    def inspect_area(self):
        """Override existing method to include object detection"""
        # Simulate object detection
        objects = self.detect_objects()
        
        # Consider it suspicious if any potentially threatening objects are detected
        suspicious_classes = ['person']
        suspicious = any(obj['class'] in suspicious_classes for obj in objects)
        
        if suspicious:
            print(f"Drone {self.id} detected suspicious objects: {objects}")
        else:
...
class Camera(ap.Agent):
    def setup(self):
        ...
        self.detection_radius = 5  # Increased detection radius
        self.last_detection_time = 0
        self.detected_robbers = []

    def detect_robbers(self):
        """Detect robbers within detection radius"""
        self.detected_robbers = []
        for robber in self.model.robbers:
            if not robber.is_caught:
                distance = np.sqrt((self.x - robber.x)**2 + (self.y - robber.y)**2)
                if distance <= self.detection_radius:
                    self.detected_robbers.append({
                        'id': robber.id,
                        'x': robber.x,
                        'y': robber.y
                    })
                    print(f"Camera {self.id} detected Robber {robber.id} at ({robber.x}, {robber.y})")
        
        return self.detected_robbers
...
class SecurityPersonnel(ap.Agent):
    def setup(self):
        ...
        self.collision_threshold = 1
        self.target_robber_coordinates = None
    
    def respond_to_camera_detection(self, detected_robbers):
        """Respond to robber detections from cameras"""
        if detected_robbers:
            # Choose the first detected robber
            target = detected_robbers[0]
            self.target_robber_coordinates = (target['x'], target['y'])
            self.chasing_robber = True
            print(f"Security {self.id} responding to robber at {self.target_robber_coordinates}")
            return True
        return False
    ...
    def patrol(self):
        """Patrol or chase robbers"""
        if self.target_robber_coordinates:
            # Move towards the target robber coordinates
            dx = 1 if self.x < self.target_robber_coordinates[0] else -1
            dy = 1 if self.y < self.target_robber_coordinates[1] else -1
            ...
            if (self.x, self.y) == self.target_robber_coordinates:
                self.target_robber_coordinates = None
                self.chasing_robber = False
        else:
            # Regular random patrol if no target
            super().patrol()
...
class SurveillanceModel(ap.Model):
    ...
    def step(self):
        ...
        for camera in self.cameras:
            camera_detections = camera.detect_robbers()
            detected_robbers.extend(camera_detections)
        
        # Update security personnel with camera detections
        for security in self.security:
            security.respond_to_camera_detection(detected_robbers)
